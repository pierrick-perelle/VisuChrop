...
function graphSetup2(parsedData){


    //TODO REFACTORING : 1 array instead of 2


    window.accesData = parsedData;
    ...
    let x = d3.scaleLinear()
        .domain([0,d3.max(parsedData[selectedChromosome].values[0].values, function (d) {
            return d.avr;
        })])
        .range([0, WIDTH]);
    ...
    function zoomed() {
        ...
        tracerCourbe(selectedChromosome,parsedData,lineGen,svg,field,field); //à chaque mouvement on redessine nos courbes.
    }
    ...
    d3.select("#floorContainer").append("select").attr("id","chromosomeSelector")
        ...
        .on("change",function(){
            ...
            tracerCourbe(selectedChromosome,parsedData,lineGen,svg,field);
        });


    parsedData.forEach(function(current_data,i){ //impossible d'utiliser .data() .enter() ici pour des raisons obscure.
    ...
    let legend = d3.select("#floorContainer").append("div").attr("id","legend").selectAll('g')
        .data(parsedData[selectedChromosome].values)
        .enter()
        .append('g')
        .attr('class', 'legend')
    ...
    legend.append('input')
        ...
        .on("input",function(){
            ...
            floorValues[origine] = this.value;
            refreshFloor(floorValues,selectedChromosome);
    ...
    svg.select("#graphlimit").selectAll(".chromosome")
        .data(parsedData) // nombre de <g id="chrx"> </g> qui vont être crée (1 par chromosome) utile pour le selecteur.
        .enter()
        .append("g")
    ...
    mouseG.append('svg:rect') // append a rect to catch mouse movements on canvas
        ...
        .on("click", function () {
            ...
            origine = getKeyByValue(field,selectedOrigin);
            //1.20 * [1 - (x/(y-4))]
            floorValues[origine] = (1-(mouse[1]/(yHeight-4))).toFixed(3); //Ajout de la valeur du seuil à notre FloorValueArray à l'index correspondant à l'origine actuellement séléctioné (selectedOrigin)
            refreshFloor(floorValues,selectedChromosome);
            ...
            mosaique(floorValues,parsedData,field);
        });
...
function refreshFloor(floorValueArray,selectedChromosome){
    ...
    Object.keys(floorValueArray).forEach(function (origineKey) {
        ...
        input.value = floorValueArray[origineKey];
    });
...
function arraySetup() {
    ...
    for (let line of lines) {
        array[line.attributes.ancestor.value] = 1/haplotype;
    }
